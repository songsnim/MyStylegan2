...
def g_path_regularize(fake_img, style, mean_path_length, decay=0.01):
    ...
    # 델연산(g(w) * y) => 합의 편미분
    grad, = autograd.grad(outputs=(fake_img * noise).sum(), inputs=style, create_graph=True)
...
def get_sample_for_log(test_image, encoder,generator, g_ema):
    with torch.no_grad():
        ...
        sample = torch.cat([test_sample[:int(args.batch/2)],test_image[:int(args.batch/2)]], dim=0)
        
        sample_ema = torch.cat([test_sample_ema[:int(args.batch/2)],test_image[:int(args.batch/2)]], dim=0)
...
def get_accuracy(test_loader, encoder, generator, predictor, device):
    ...
    with torch.no_grad():
        for idx,(image, label) in enumerate(test_loader):
            ...
            prediction = P_pred.max(1,keepdim=True)[1] # output에서 제일 큰 놈의 index를 반환한다(이경우에 0 or 1)
...
def train(args, train_loader, test_loader, encoder, generator, discriminator, predictor,
    ...
    test_image = test_image.to(device) ; test_label = test_label.to(device)
    ...
    for idx in pbar:
        ...
        if d_regularize:
            ...
            discriminator.zero_grad()
            r1_loss_backward = (args.r1 / 2 * r1_loss * args.d_reg_every + 0 * real_pred[0])
        ...
        if g_regularize:
            ...
            for idx,(style, mean_path_length) in enumerate(zip(styles, mean_path_lengths)):
                path_loss, mean_path_length_output, path_lengths = g_path_regularize(fake_img, style, mean_path_length)
        ...
        if get_rank() == 0:
            ...
            if i % 100 == 0:
                sample, sample_ema = get_sample_for_log(test_image, encoder, generator, g_ema)
                sample = wandb.Image(sample)
                sample_ema = wandb.Image(sample_ema)  
                wandb.log({"G, test": sample,
                           "g_ema, test": sample_ema})
                p_accuracy = get_accuracy(test_loader, encoder, generator, predictor,device)
                wandb.log({"Accuracy": p_accuracy})
                
                    
            if i % 1000 == 0:
                os.makedirs(f"checkpoint/{today}_{args.description}/", exist_ok=True)
                torch.save(
                    {
