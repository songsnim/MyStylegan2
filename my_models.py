        ...
        if not self.fused:
            ...
            if self.demodulate:
                w = weight.unsqueeze(0) * \
                    stylespace.view(batch, 1, in_channel, 1, 1) # style로 conv weight scaling
...
class MapToStyleConv(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, stride, padding):
        super().__init__()
        self.conv = EqualConv2d(in_channel, out_channel, kernel_size=kernel_size, stride=stride, padding=padding)
...
class Generator(nn.Module):
    def __init__(self,
        ...
        for idx, feat in enumerate(self.feat_list):
            ...
            if h >= 4 and w >= 4:
                ...
                for i in range(num_conv_layers - 1):
                    map_to_style.append(MapToStyleConv(c, style_dim, kernel_size=3, stride=2, padding=1))
                    c = style_dim
                map_to_style.append(MapToStyleConv(c, style_dim, kernel_size=2, stride=1, padding=0))
                map_to_style.append(MapToStyleConv(c, style_dim, kernel_size=1, stride=1, padding=0))
        ...
        for i in range(3, self.log_size + 1):
            out_channel = self.channels[2**i] # init 이후 8 resolution부터 StyleConv용 channel
    ...
    def forward(
        ...
        if noise is None:
            if randomize_noise:
                ...
                noise = [getattr(self.noises, f"noise_{i}") for i in range(self.layer_indices)]

        if input_type == 'feat_list':
            styles = self.get_space_from_feat(feat_list)
        elif input_type == 'styles':
            styles = feat_list
        
        out = self.input(styles[0]) # latent와 동일한 batch로 constant input 생성  
        out = self.conv_init(out, styles[0], noise=noise[0])
        skip = self.to_rgb_init(out, styles[1])
